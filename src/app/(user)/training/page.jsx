  ...
  const fetchSearchResults = async () => {
    ...
    try {
      const res = await fetch(
        ...
        }/api/training?search=${encodeURIComponent(searchQuery)}`
      );
      const data = await res.json();
  ...
  return (
    <div className="flex flex-col w-full max-w-screen mx-auto min-h-screen px-6 py-12">
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-4 sm:gap-x-6 lg:gap-x-8 pt-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="pt-8">
          <NotFound
            message="We couldn’t find any training matching your search. Try different keywords."
            buttons={[]}
          />
        </div>
      ) : null}
