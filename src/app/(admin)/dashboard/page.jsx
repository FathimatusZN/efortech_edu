...
const DashboardAdmin = () => {
  ...
  const CustomXAxisTick = ({ x, y, payload }) => {
    ...
    const text =
      fullText.length > maxTotalChars
        ? fullText.slice(0, maxTotalChars) + "..."
        : fullText;
    ...
    const fetchTodoCounts = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/todo`
        );
        ...
        // ========== PART 2 : Top Training ==========
        const resTrainings = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/top-trainings`
        );
        setTopTrainings(resTrainings.data.data);

        // ========== PART 3 : Monthly Registration ==========
        const resMonthly = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/monthly-registrations`
        );
        ...
        const resTrainingOverview = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/training-overview`
        );
        ...
        // ========== PART 5 : Global Stat ==========
        const resGlobalStats = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dashboard/summary`
        );
        ...
        <div className="bg-white p-2 border rounded shadow text-sm">
          ...
          {payload.map((entry, index) => (
            <p
              ...
              style={{ color: colorMap[entry.dataKey] || "#000" }}
            >
    ...
    <ProtectedRoute allowedRoles={["admin", "superadmin"]}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <Link key={idx} href={card.href}>
                <div
                  className="cursor-pointer h-[200px] bg-white bg-opacity-10 backdrop-blur-md rounded-xl flex flex-col items-center justify-center p-4 shadow-[inset_4px_2px_15px_rgba(255,255,255,0.4)]
      transition-transform duration-300 ease-in-out transform hover:scale-[1.05] hover:shadow-[0_10px_25px_rgba(255,255,255,0.2)] hover:backdrop-blur-xl"
                >
                  ...
                  <p className="text-gray-200 text-5xl sm:text-6xl font-bold drop-shadow-md text-center">
                    {card.value}
                  </p>
        ...
        <div className="w-full max-w-6xl mx-auto mt-6 bg-white border border-lightBlue rounded-lg shadow-md cursor-pointer p-4">
          ...
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              <Bar
                stackId="cert"
                dataKey="valid_certificate"
                fill="#ED7117"
                name="Valid Certificate"
              />
              <Bar
                stackId="cert"
                dataKey="expired_certificate"
                fill="#FCAE1E"
        ...
        <div
          ...
          onKeyDown={(e) =>
            e.key === "Enter" && setShowGlobalStats(!showGlobalStats)
          }
        >
          <div className="flex items-center justify-center">
            <h2 className="text-lg font-semibold text-gray-600 hover:text-black">
              Quick Global Review
            </h2>
          </div>
          {showGlobalStats && (
