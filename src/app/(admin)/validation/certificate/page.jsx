...
import { dummyCertificateData } from "@/app/(admin)/validation/Data";
import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
...
const ValidationCertificatePage = () => {
  ...
  const needToBeProcessed = certificateData.filter(item => !item.validation);
  const processedData = certificateData.filter(item => item.validation);
  ...
  return (
    <ProtectedRoute allowedRoles={["admin", "superadmin"]}>
      <div className="max-w-screen mx-auto p-4 md:p-6">
        <h1 className="text-xl md:text-2xl font-bold text-left mb-6">Training Certificate Validation</h1>
                  ...
                  <div className="absolute top-full z-50 mt-2 right-0 bg-white border border-gray-300 rounded-xl shadow-md w-40">
                    ...
                    <div className="border-t border-gray-300">
                      <p
                        ...
                        onClick={() => {
                          setCertificateData([...needToBeProcessed].sort((a, b) => a.fullName.localeCompare(b.fullName)));
                      ...
                      <p
                        className="cursor-pointer hover:bg-gray-200 text-mainBlue p-2"
                        onClick={() => {
                          setCertificateData([...needToBeProcessed].sort((a, b) => new Date(a.requestDate) - new Date(b.requestDate)));
                          setIsFilterOpen(false);
                        }}
                      >
                        Request Date
                      ...
                      <p
                        className="cursor-pointer hover:bg-gray-200 text-mainBlue p-2"
                        onClick={() => {
                          setCertificateData([...needToBeProcessed].sort((a, b) => new Date(a.issuedDate) - new Date(b.issuedDate)));
                          setIsFilterOpen(false);
                        }}
                      >
                        Issued Date
