...
const ValidationPage = () => {
  ...
  return (
    <div className="max-w-screen mx-auto px-4 sm:px-6 md:px-8 py-6 space-y-12">
      <h1 className="text-xl md:text-2xl font-bold text-left mb-6">Validation</h1>
      ...
      <div className="bg-white outline outline-3 outline-mainBlue rounded-2xl p-6 mb-6 shadow-[8px_8px_0px_0px_#157ab2]">
              ...
              {isFilterOpen && (
                <div className="absolute top-full z-50 mt-2 right-0 bg-white border border-gray-300 rounded-xl shadow-md w-40">
                  ...
                  <div className="border-t border-gray-300">
                    <p className="cursor-pointer hover:bg-gray-200 text-secondBlue p-2" onClick={() => setIsFilterOpen(false)}>Name</p>
                    <p className="cursor-pointer hover:bg-gray-200 text-secondBlue p-2" onClick={() => setIsFilterOpen(false)}>Request Date</p>
                    <p className="cursor-pointer hover:bg-gray-200 text-secondBlue p-2" onClick={() => setIsFilterOpen(false)}>Issued Date</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[768px] border-collapse rounded-lg overflow-hidden">
            <thead>
      ...
      <div className="bg-white outline outline-3 outline-mainBlue shadow-[8px_8px_0px_0px_#157ab2] rounded-2xl p-6">
        <div className="relative">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold">Certificate Validation</h2>
            <div className="relative" ref={filterRef}>
              {!isFilterOpen && (
                <Image
                  src={filterIcon}
              ...
              {certificateData.length > 0 ? (
                certificateData.map((item) => (
                  <tr key={item.id} className="text-center border-t">
                    ...
                    <td className="p-3 border-2 border-lightBlue text-center">
                      <button onClick={() => handleValidation("certificate", item.id)}>
                        <Image src={checkIcon} alt="Approve" width={40} height={40} />
                      </button>
                    </td>
                    <td className="p-3 border-2 border-lightBlue text-center">
